"""
QRコード作成ツール(CMYK出力)
pyinstaller app_V2.py --onefile --noconsole --add-data "python_312\Lib\site-packages\customtkinter;customtkinter"
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.title("QRコード作成ツール（YMCKバージョン）")
    ...
    def make_qr_all(self):
        ...
        if file_path == "":
            ...
            try:
                ...
                messagebox.showerror(
                    title="エラー", message="ファイルを読み込めませんでした。パスと形式を確認してください。",
                )
                return
            ...
            for i in range(2, last_row + 1):
                if (sheet.cell(i, 1).value is None) or (sheet.cell(i, 2).value is None):
                    flag = False
                    break
